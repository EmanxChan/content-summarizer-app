  ...
  async function run() {
    ...
    try {
      ...
      fetch('/api/history', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'article', title: data.title, url: url.trim(), summary: data.summary, insights: data.insights, highlights: data.highlights, next_steps: data.next_steps }) })
    } catch (err: unknown) {
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <input
          ...
          placeholder="Custom focus e.g. 'focus on key arguments' (optional)"
